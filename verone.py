...
class BarrelsClientApp(tk.Tk):
    ...
    def init_ui(self):
        ...
        # Account Type
        tk.Label(settings_frame, text="ACCOUNT TYPE", font=("Arial", 8, "bold"), bg=THEME['sidebar'], fg=THEME['text_secondary']).pack(anchor="w", padx=5, pady=(5,0))
        tk.Radiobutton(settings_frame, text="Offline", variable=self.account_type, value="offline", bg=THEME['sidebar'], fg=THEME['text'],
                       selectcolor=THEME['sidebar'], activebackground=THEME['sidebar'], activeforeground=THEME['text']).pack(anchor="w", padx=5)
        tk.Radiobutton(settings_frame, text="Mojang Account", variable=self.account_type, value="mojang", bg=THEME['sidebar'], fg=THEME['text'],
                       selectcolor=THEME['sidebar'], activebackground=THEME['sidebar'], activeforeground=THEME['text']).pack(anchor="w", padx=5)
        ...
        tk.Label(self.username_frame, text="USERNAME", font=("Arial", 8, "bold"), bg=THEME['sidebar'], fg=THEME['text_secondary']).pack(anchor="w")
        self.username_input = tk.Entry(self.username_frame, font=("Arial", 10), bg=THEME['input_bg'], fg=THEME['text'],
        ...
        tk.Label(self.mojang_frame, text="EMAIL", font=("Arial", 8, "bold"), bg=THEME['sidebar'], fg=THEME['text_secondary']).pack(anchor="w")
        self.email_input = tk.Entry(self.mojang_frame, font=("Arial", 10), bg=THEME['input_bg'], fg=THEME['text'],
                                    insertbackground=THEME['text'], bd=0, highlightthickness=0)
        self.email_input.pack(fill="x", pady=(3, 0))
        tk.Label(self.mojang_frame, text="PASSWORD", font=("Arial", 8, "bold"), bg=THEME['sidebar'], fg=THEME['text_secondary']).pack(anchor="w")
        self.password_input = tk.Entry(self.mojang_frame, font=("Arial", 10), bg=THEME['input_bg'], fg=THEME['text'],
        ...
        changelog_items = [
            "🛢️ Rebranded to Barrels Client with new theme.",
            "⚙️ Added automatic Java 21 installation.",
            "✨ Improved UI styling and layout.",
            "🔒 Added Mojang account authentication."
        ]
    ...
    def authenticate_mojang(self, email, password):
        ...
        payload = {"agent": {"name": "Minecraft", "version": 1}, "username": email, "password": password, "requestUser": True}
    ...
    def prepare_and_launch(self):
        ...
        def launch_thread():
            ...
            cmd = self.build_launch_command(selected_version, username, uuid, access_token, user_type, ram, java_path, ai_mode=self.ai_mode.get())
            if cmd:
                try:
